...
import { BaseAgent, BusinessData, HousingData } from './base.agent.js';
...
export class AgentOrchestrator {
  ...
  private initializeAgents(): void {
    const defaultConfig = {
      // Each concrete agent (GooglePlacesAgent, HousingScannerAgent, ...)
      // overrides `name` in its own constructor, but AgentConfig requires
      // it up front, so this placeholder just needs to satisfy the type —
      // it's never the name that actually gets used.
