/**
 * Agent 3: Services & Professionals Scanner
 * Specialized in finding service providers (plumbers, electricians, cleaners, etc.)
 */

import { BaseAgent, AgentConfig, BusinessData, ScoutingResult } from './base.agent.js';

export class ServicesScannerAgent extends BaseAgent {
  ...
  async scan(location: { lat: number; lng: number; radiusKm: number }): Promise<ScoutingResult> {
    ...
    try {
      ...
      // Search for service provider types
      const serviceTypes = [
        'plumber', 'electrician', 'carpenter', 'painter',
        'cleaning_service', 'moving_company', 'locksmith',
        'pest_control', 'landscaper', 'contractor',
        'hair_care', 'spa', 'massage', 'tattoo',
        'veterinary_care', 'pet_grooming', 'pet_sitter',
        'laundry', 'dry_cleaning', 'tailor',
        'photographer', 'event_planner', 'florist',
        'real_estate_agency', 'insurance_agency', 'travel_agency',
        'legal_services', 'accounting', 'consultant'
      ];
      ...
      for (const type of serviceTypes) {
        try {
          ...
          for (const place of results) {
            const business = await this.fetchServiceDetails(place.place_id);
            if (business) {
  ...
  private async fetchServiceDetails(placeId: string): Promise<BusinessData | null> {
    try {
      await this.rateLimit();
      ...
      const category = this.categorizeService(result.types);
      ...
      return {
        ...
        images: result.photos?.slice(0, 8).map((p: any) => 
        ...
        response_time_minutes: this.estimateResponseTime(result.types),
        tags: result.types || [],
        opening_hours: this.parseOpeningHours(result.opening_hours),
        amenities: this.extractServiceAmenities(result),
      };
    } catch (error) {
      console.error(`Error fetching service details for ${placeId}:`, error);
      return null;
    }
  }

  private categorizeService(types: string[]): { main: string; sub?: string } {
    const mapping: Record<string, { main: string; sub?: string }> = {
      ...
      carpenter: { main: 'Home Services', sub: 'Carpentry' },
      painter: { main: 'Home Services', sub: 'Painting' },
      cleaning_service: { main: 'Home Services', sub: 'Cleaning' },
      moving_company: { main: 'Home Services', sub: 'Moving' },
      locksmith: { main: 'Home Services', sub: 'Locksmith' },
      pest_control: { main: 'Home Services', sub: 'Pest Control' },
      landscaper: { main: 'Home Services', sub: 'Landscaping' },
      contractor: { main: 'Home Services', sub: 'General Contracting' },
      hair_care: { main: 'Beauty & Personal Care', sub: 'Hair Salon' },
      spa: { main: 'Beauty & Personal Care', sub: 'Spa' },
      massage: { main: 'Beauty & Personal Care', sub: 'Massage' },
      tattoo: { main: 'Beauty & Personal Care', sub: 'Tattoo' },
      veterinary_care: { main: 'Pet Services', sub: 'Veterinary' },
      pet_grooming: { main: 'Pet Services', sub: 'Grooming' },
      laundry: { main: 'Personal Services', sub: 'Laundry' },
      dry_cleaning: { main: 'Personal Services', sub: 'Dry Cleaning' },
      tailor: { main: 'Personal Services', sub: 'Tailoring' },
      photographer: { main: 'Creative Services', sub: 'Photography' },
      event_planner: { main: 'Creative Services', sub: 'Event Planning' },
      florist: { main: 'Retail', sub: 'Florist' },
      real_estate_agency: { main: 'Real Estate', sub: 'Agency' },
      insurance_agency: { main: 'Finance', sub: 'Insurance' },
      legal_services: { main: 'Professional Services', sub: 'Legal' },
    ...
    return { main: 'Services', sub: types[0] };
  }

  private estimateResponseTime(types: string[]): number {
    const emergencyServices = ['plumber', 'electrician', 'locksmith'];
    if (types.some(t => emergencyServices.includes(t))) return 30;
    return 120;
  }
  ...
  private calculateTrustScore(place: any): number {
    ...
    if (place.reviews?.some((r: any) => r.relative_time_description)) score += 5;
  ...
  private extractServiceAmenities(place: any): string[] {
    const amenities: string[] = [];
    
    if (place.wheelchair_accessible_entrance) amenities.push('Wheelchair accessible');
    if (place.accepts_credit_cards !== false) amenities.push('Credit cards accepted');
    if (place.appointment_required === false) amenities.push('Walk-ins welcome');
    if (place.free_wifi) amenities.push('Free Wi-Fi');
    if (place.parking) amenities.push('Parking available');
    if (place.delivery) amenities.push('Delivery available');
    if (place.online_appointments) amenities.push('Online booking');
