...
export class HousingService {
  ...
  create(listing: Omit<HousingListing, 'id' | 'created_at' | 'updated_at'>): HousingListing {
    ...
    // FIX: same data-loss bug as business.service.ts — rating/review_count
    // were hardcoded to 0/0 in the VALUES list, discarding real scanned
    // Google ratings, and is_active was hardcoded to 1 instead of respecting
    // the caller.
    stmt.run(
      id,
      listing.host_id,
  ...
  /**
   * Upsert a listing coming from the scouting agents, keyed on the agent's
   * deterministic id (e.g. `hsg_<google_place_id>`) so re-scanning an area
   * updates existing rows instead of duplicating every hotel on every scan.
   */
  upsertFromScout(listing: HousingListing): HousingListing {
    ...
    if (existing) {
      const stmt = db.prepare(`
        UPDATE housing_listings SET
          title = ?, rating = ?, review_count = ?, images = ?,
          price_per_night = ?, is_active = ?, updated_at = ?
        WHERE id = ?
      `);
      stmt.run(
        listing.title,
      ...
      return { ...existing, ...listing, updated_at: now };
    }
    ...
    return { ...listing, created_at: now, updated_at: now };
  }
