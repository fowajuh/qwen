...
let RecommendationsService = RecommendationsService_1 = class RecommendationsService {
    constructor(prisma) {
        ...
        this.genAI = process.env.EXPO_PUBLIC_GEMINI_API_KEY
            ? new generative_ai_1.GoogleGenerativeAI(process.env.EXPO_PUBLIC_GEMINI_API_KEY)
            : null;
        this.breaker = new opossum_1.default((prompt) => this.callModel(prompt), { timeout: 8000, errorThresholdPercentage: 50, resetTimeout: 15000 });
    ...
    async callModel(prompt) {
        ...
        const model = this.genAI.getGenerativeModel({ model: 'gemini-1.5-flash' }, { baseUrl: process.env.EXPO_PUBLIC_GEMINI_API_BASE });
    ...
    async getRecommendations(tripId, interests, budgetStyle, destinationCode) {
        const contextHash = (0, crypto_1.createHash)('sha1')
        ...
        const prompt = `For a trip with interests=${interests.join(',')} and budget style=${budgetStyle}${destinationCode ? `, destination=${destinationCode}` : ''}, suggest 8 real, specific places/restaurants/activities.
...
Return ONLY valid JSON (no markdown, no extra text):
{
  "places": [
      ...
      "searchQuery": "google maps search query for this place"
        ...
        try {
            ...
            if (result?.places) {
                ...
                recommendations = await Promise.all(result.places.slice(0, 8).map((p) => this.enrichPlaceWithRealData(p, interests)));
    ...
    async fallbackWithGooglePlaces(interests, budgetStyle, destinationCode) {
    ...
    async enrichPlaceWithRealData(placeData, interests) {
        // Resolve category early so it's available in all branches
        const matchingInterest = interests.find((i) => placeData.type?.toLowerCase().includes(i)) || placeData.type || 'see';
        try {
            const apiKey = process.env.GOOGLE_PLACES_API_KEY;
            if (!apiKey) {
            ...
            const matchingInterest = interests.find((i) => placeData.type.toLowerCase().includes(i)) || placeData.type;
            return {
                title: placeData.name,
                category: matchingInterest,
                blurb: `${placeData.name} in ${placeData.city}, ${placeData.country}`,
    ...
    generatePlacePhotoUrl(query) {
        ...
        return `https://images.unsplash.com/photo-${Math.floor(Math.random() * 100000)}?w=400&h=300&fit=crop`;
    }
    ...
    generateRealisticCoord(type, city) {
    ...
    ruleBasedFallback(interests, budgetStyle, destinationCode) {
