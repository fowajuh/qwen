...
import {
  ...
  Laugh, Flame, Zap, Check, CheckCheck, File, PhoneCall
} from "lucide-react";
...
function ChatDetail() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: "m1", text: "Hey! Are we still on for the meeting later today?", from: "other", time: "8:14 AM", read: true },
    { id: "m2", text: "Yes absolutely. 3pm works perfectly for me. See you then!", from: "me", time: "8:16 AM", read: true },
    { id: "m3", text: "See you at 3pm!", from: "other", time: "9:22 AM", read: true },
  ]);
  ...
  const sendText = (e: React.FormEvent) => {
    ...
    setMessages((p) => [...p, { id: Date.now().toString(), text: inputText, from: "me", time: now }]);
    setInputText("");
    setIsTyping(true);
    // Simulate reply
    setTimeout(() => {
      setIsTyping(false);
      setMessages((p) => [...p, { id: (Date.now() + 1).toString(), text: "Got it, thanks!", from: "other", time: new Date().toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" }) }]);
    }, 1500);
  };
  ...
  const sendVoice = (blob: Blob, dur: number) => {
    ...
    const now = new Date().toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" });
    setMessages((p) => [...p, { id: Date.now().toString(), audio: { url, duration: dur }, from: "me", time: now }]);
